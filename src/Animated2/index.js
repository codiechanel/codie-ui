...
class App extends React.Component {
  ...
  restart(elem) {
    ...
    if (!this.animating) {
      this.animating = true
      var me = elem;
      me.className = `magictime ${this.props.variant}`;
   ...
   * TODO
   * need to change this
   * React will call this if it is reusing components
   * like in an array
   * need checking if we really want to animate
   * this fixes onEnter events so we could reanimate
   * @param {obj} nextProps 
   * @param {obj} nextState 
   */
  ...
  render() {
    ...
    let divClassName = `magictime ${this.props.variant}`;
...
App.propTypes = {
  ...
  variant: PropTypes.oneOf(["bounce", "lightSpeedIn", "zoomIn", "slideInUp", 
  // magictime
  "puffIn", "boingInUp", 
  // loader
  "ball-pulse"]).isRequired,
